...
class BankInfoService
{
     ...
     * @return
     */
    public static function getNameOfBank(string $cardNo)
    {
        // 验证银行卡号格式
        $checkBank = self::checkBank($cardNo);
        if (!$checkBank)
            return '银行卡号错误';

        return self::bankSearch($cardNo) ?? false;
    }
     ...
     * @return
     */
    public static function bankSearch(string $cardNo)
    {
        $cardPosition = [4,5,6,8]; //需要查找的位数

        foreach ($cardPosition as $offset) {
            $card = substr($cardNo , 0, $offset);
            if (isset(self::$bankList[$card]))
                return self::$bankList[$card];
        }
     ...
     * @return
     */
    public static function checkBank(string $cardNo)
    {
